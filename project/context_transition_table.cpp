...
void ContextTransitionTable<M, N, A>::addTransition(CallSite<M, N, A> call_site, Context<M, N, A> target_context) {
	if(!target_context.is_null) {
			...
			default_call_sites.insert(call_site);
	}
}

template<class M, class N, class A>
std::unordered_map<Context<M, N, A>, std::unordered_set<CallSite<M, N, A>>> ContextTransitionTable<M, N, A>::getCallers(void) {
	return callers;
}
...
template<class M, class N, class A>
std::unordered_map<Context<M, N, A>, std::unordered_set<CallSite<M, N, A>>> ContextTransitionTable<M, N, A>::getCallSitesOfContexts(void) {
	return call_sites_of_contexts;
}

template<class M, class N, class A>
std::unordered_set<CallSite<M, N, A>> ContextTransitionTable<M, N, A>::getDefaultCallSites(void) {
